export default {
  app: {
    close: 'Cerrar',
    saved: 'Imagem salva',
    alpha: 'versão alfa',
    continents: {
      EU: 'Europa',
      AS: 'Asia',
      AM: 'America',
      AFAU: 'África y Australia',
      OT: 'Otras banderas'
    },
    continentLabel: 'Continente'
  },
  poster: {
    title: 'Esta es la versión alfa',
    text: `Sí, está sucediendo. Después de 3 años encontré un momento para lidiar con esto.
    solicitud. Esta es la <b> primera versión pública </b> con base
    mecánicas, como seleccionar países, ojos, accesorios y guardarlos
    todas.
    <br>
    <br>
    Hay muchas cosas que debo implementar, así que me alegraré,
    si me escribe en el correo electrónico que aparece en Google Play y da algún comentario útil
    Además, si puedes dibujar accesorios realmente bonitos, escríbeme también :)`,
    action: '¡Vamos!'
  },
  privacy: {
    title: 'Política de privacidad',
    text: 'Antes de utilizar esta aplicación, debe leer y aceptar la política de privacidad.',
    agree: 'He leido y estoy de acuerdo con la politica de privacidad',
    action: 'Ok'
  }
}
